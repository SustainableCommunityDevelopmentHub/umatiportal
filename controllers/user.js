...
exports.getUpdateProfileAjax = function (req, res, next) {
    ...
    if (item === "offer_buildingsupplies") var data = { offer_buildingsupplies: val };
    if (item === "need_foodbox") var data = { need_foodbox: val };
    if (item === "offer_foodbox") var data = { offer_foodbox: val };
    if (item === "need_foodbox1") var data = { need_foodbox1: val };
    if (item === "offer_foodbox1") var data = { offer_foodbox1: val };
    if (item === "need_foodbox2to3") var data = { need_foodbox2to3: val };
    if (item === "offer_foodbox2to3") var data = { offer_foodbox2to3: val };
    if (item === "need_foodbox4to5") var data = { need_foodbox4to5: val };
    if (item === "offer_foodbox4to5") var data = { offer_foodbox4to5: val };
    if (item === "need_foodbox6to8") var data = { need_foodbox6to8: val };
    if (item === "offer_foodbox6to8") var data = { offer_foodbox6to8: val };
    if (item === "need_compost") var data = { need_compost: val };
    if (item === "offer_compost") var data = { offer_compost: val };
    if (item === "need_compostpickup") var data = { need_compostpickup: val };
    if (item === "offer_compostpickup") var data = { offer_compostpickup: val };
    if (item === "need_householditems") var data = { need_householditems: val };
    if (item === "offer_householditems") var data = { offer_householditems: val };
    if (item === "need_tools") var data = { need_tools: val };
    if (item === "offer_tools") var data = { offer_tools: val };
    if (item === "need_clothing") var data = { need_clothing: val };
    if (item === "offer_clothing") var data = { offer_clothing: val };
    if (item === "need_books") var data = { need_books: val };
    if (item === "offer_books") var data = { offer_books: val };
    if (item === "need_plants") var data = { need_plants: val };
    if (item === "offer_plants") var data = { offer_plants: val };
    if (item === "need_catfood") var data = { need_catfood: val };
    if (item === "offer_catfood") var data = { offer_catfood: val };
    if (item === "need_dogfood") var data = { need_dogfood: val };
    if (item === "offer_dogfood") var data = { offer_dogfood: val };
